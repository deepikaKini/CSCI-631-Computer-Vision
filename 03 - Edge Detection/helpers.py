...
def non_maximal_suppression(values: np.ndarray,
    ...
    #for each pixel (value), apply the filter to it
    for i in range(height):
        for j in range(width):
            value = values[i, j]

            # Define the neighborhood boundaries according to kernel size
            min_row = max(0, i - window // 2)
            max_row = min(height, i + window // 2 + 1)
            min_col = max(0, j - window // 2)
            max_col = min(width, j + window // 2 + 1)

            # Check if current value is the maximum in its neighborhood
            is_local_max = True
            ...
            # If 'value' is a local maximum, set it to value else black
            if is_local_max:
                out[i, j] = value
            else:
                out[i, j] = 0
